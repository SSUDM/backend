...
public class JwtTokenFilter extends OncePerRequestFilter { //OncePerRequestFilter : 매번 들어갈 때 마다 체크 해주는 필터
    @Autowired private final RegisterAndLoginService registerAndLoginService;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        UsernamePasswordAuthenticationToken authenticationToken = new UsernamePasswordAuthenticationToken(
                findByLoginEmail.getEmail(), null, List.of(new SimpleGrantedAuthority(findByLoginEmail.getUserRole().name())));
